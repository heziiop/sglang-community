...
def maybe_apply_quarot_anti_rotation(model_path: str, draft_model) -> bool:
    ...
    Q = _load_quarot_rotation_matrix(rotation_path).to(torch.float32)
    ...
    Q_k = torch.block_diag(*[Q] * num_aux).to(draft_model_inner.fc.weight.device)
    ...
    logger.info(
        "Applied QuaRot anti-rotation to draft model: "
        "fc (Q <%s>, num_aux=%d).",
